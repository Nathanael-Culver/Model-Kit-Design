...
def instance(c,lid,yp,hh,x,y,sch_uuid,tagbase):
    ...
    for p in c['pins']:
        ...
        if c.get('status') in ('dnp','external') or p.get('status')=='nc':
            ...
            labs.append(label(p['net'],px,py,side,tagbase+':label:'+p['number']))
    if c.get('note'):
...
def layout_grid(comps, origin_x, origin_y, cols=4, dx=58, dy=45):
    ...
    for i,c in enumerate(comps):
        ...
        positions.append((c,snap(origin_x+col*dx),snap(origin_y+row*dy)))
    return positions
...
def make_flat(comps):
    ...
    for key,items in groups.items():
        ...
        o.append(f' (text "{headers[key]}" (at {snap(ox-10):.2f} {snap(oy-15):.2f} 0) {eff(1.5)} (uuid {U("flat:header:"+key)}))')
